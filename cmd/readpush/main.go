...
// PushAnswer is the response returned for each pending notification.
...
// sqlGetNotification fetches the oldest unsent notification for brigades
// belonging to the given partner. Updating last_try prevents double-delivery
// when multiple bot instances poll concurrently.
...
JOIN
	head.free_telegram_ids ft ON pm.brigade_id = ft.brigade_id
JOIN
